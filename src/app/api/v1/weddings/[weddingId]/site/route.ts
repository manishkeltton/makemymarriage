...
import { updateSiteSchema } from "@/modules/website/validation/wedding-site.schemas";
...
export async function GET(
  ...
  try {
    ...
    const result = await WeddingSiteService.getOrCreateSite(weddingId, session.user.id);
    ...
    console.error("GET Wedding Site Error:", err);
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const result = await WeddingSiteService.updateSite(
      weddingId,
      session.user.id,
      parseResult.data
    );
    ...
    if (!result.success) {
      const status =
        result.code === "FORBIDDEN"
          ? 403
          : result.code === "SLUG_TAKEN" || result.code === "INVALID_SLUG"
          ? 409
          : 400;
      return NextResponse.json(
        { success: false, error: { code: result.code || "BAD_REQUEST", message: result.error } },
    ...
    console.error("PATCH Wedding Site Error:", err);
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: "Internal server error" } },
      { status: 500 }
    );
