...
import { updateTaskSchema } from "@/modules/tasks/validation/task.schemas";
...
export async function GET(
  ...
  try {
    ...
    const result = await TaskService.getTaskById(weddingId, taskId, session.user.id);
...
export async function PATCH(
  ...
  try {
    ...
    const result = await TaskService.updateTask(weddingId, taskId, session.user.id, parseResult.data);
    if (!result.success) {
      const statusCode =
...
export async function DELETE(
  ...
  try {
    ...
    const { weddingId, taskId } = await params;
    const result = await TaskService.deleteTask(weddingId, taskId, session.user.id);
